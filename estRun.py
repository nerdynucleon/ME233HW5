...
def max_diff_angles(angles):
    ...
    # return largest distance between two angles in numpy array of angles
...
def estRun(time, dt, internalStateIn, steeringAngle, pedalSpeed, measurement):
    ...
    if not (np.isnan(measurement[0]) or np.isnan(measurement[1])):
        ...
        # GPS is zero mean
        # GPS_VAR = np.diag([1.08807010408, 2.98447239424])
        ...
        # Calculate Roughening Statistics
        ...
        std_x_rough = K * (N ** (-1.0/d)) * np.abs(np.max(x) - np.min(x))
        std_y_rough = K * (N ** (-1.0/d)) * np.abs(np.max(y) - np.min(y))
        std_theta_rough = K * (N ** (-1.0/d)) * max_diff_angles(theta)
